    ...
    compras.map((val) => {
        containerProdutos.innerHTML += `
            <div class="produto-single">
                ...
                <p>Salto: ${val.Salto}</p>
                <p>${val.informacao}</p>
                <button onclick="addToCart(${val.id})">Adicionar ao carrinho</button>
            </div>
        `;
    });
}
...
function pesquisarProdutos() {
    ...
    var produtosFiltrados = compras.filter((produto) => {
...
function atualizarCarrinho() {
        ...
        if (val.quantidade > 0) {
            containerCarrinho.innerHTML += `
                <li>
                    <img src="${val.img}" alt="${val.nome}" />
                    <p>${val.nome}</p>
                    <p>Quantidade de sapato :${val.quantidade} </p><p> R$ ${val.preco.toFixed(2).replace(".", ",")}</p>
    ...
    quantityElement.textContent = `Quantidade de Produtos: ${quantity}`;
